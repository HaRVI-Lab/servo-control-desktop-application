...
#include "ui_pushsubwindow.h"

PushSubWindow::PushSubWindow(MainWindow *mainWinPtr, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::PushSubWindow),
...
void PushSubWindow::on_angle_slider_valueChanged(int value)
{
    ui->angle_val->setText(QString::number(value));
}

void PushSubWindow::on_velocity_slider_valueChanged(int value)
{
    ui->velocity_val->setText(QString::number(value));
}

void PushSubWindow::on_reset_btn_clicked()
{
    ...
    mainWindowPtr->servoUtility.resetPositionCustom(mainWindowPtr->dxl_ids, 1, firstReset);
    firstReset = 0;
...
void PushSubWindow::on_start_btn_clicked()
{
    ui->start_btn->setEnabled(false);
...
void PushSubWindow::on_quit_btn_clicked()
{
    ui->start_btn->setEnabled(false);
    if (firstReset == 1) {
        // Change servos velocity
        ...
        qDebug() << "Resetting servos positions...";
        mainWindowPtr->servoUtility.quitCustom(mainWindowPtr->dxl_ids, 1);
        firstReset = 1;
        qDebug() << "Servos positions have been reset!";
        for (int i = 0; i < NUM_OF_DXL; i++) {
            mainWindowPtr->servoUtility.disableTorque(mainWindowPtr->dxl_ids[i]);
