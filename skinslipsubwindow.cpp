...
#include "ui_skinslipsubwindow.h"

SkinSlipSubWindow::SkinSlipSubWindow(MainWindow *mainWinPtr, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::SkinSlipSubWindow),
...
void SkinSlipSubWindow::on_angle_slider_valueChanged(int value)
{
    ui->angle_val->setText(QString::number(value));
}

void SkinSlipSubWindow::on_velocity_slider_valueChanged(int value)
{
    ui->velocity_val->setText(QString::number(value));
}

void SkinSlipSubWindow::on_reset_btn_clicked()
{
...
void SkinSlipSubWindow::on_start_btn_clicked()
{
    ui->start_btn->setEnabled(false);
...
void SkinSlipSubWindow::on_quit_btn_clicked()
{
    ui->start_btn->setEnabled(false);
    // Change servos velocity
    for (int i = 0; i < NUM_OF_DXL; i++) {
